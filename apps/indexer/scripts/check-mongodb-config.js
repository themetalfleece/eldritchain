...
async function checkMongoDBConfig() {
  ...
  try {
    ...
    try {
      ...
      if (ismaster.setName) {
        ...
        if (ismaster.setName === config.replicaSetName) {
          ...
          console.log(
            `⚠️  Replica set name mismatch! Expected: ${config.replicaSetName}, Found: ${ismaster.setName}`
          );
        }
      } else {
    ...
    try {
      ...
      console.log(
        `   - Primary: ${status.members.find((m) => m.stateStr === "PRIMARY")?.name || "None"}`
      );
      console.log(
        `   - State: ${status.myState === 1 ? "Primary" : status.myState === 2 ? "Secondary" : "Other"}`
      );
    } catch (error) {
